...
describe('ShaderPortingTab', () => {
  it('analyzes, verifies, and invalidates a single-pass recipe', async () => {
    ...
    await user.clear(screen.getByLabelText('glow (float)'))
    await user.type(screen.getByLabelText('glow (float)'), '0.8')
    expect(screen.getByRole('heading', { name: /React 接入配方/ })).toBeInTheDocument()
    expect((screen.getByLabelText('2. Uniform bindings 可复制文本') as HTMLTextAreaElement).value).toContain('"value": 0.8')
    fireEvent.change(screen.getByLabelText('Shadertoy Image 源码'), { target: { value: `${readySource}\n// changed` } })
